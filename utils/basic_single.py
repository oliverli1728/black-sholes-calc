...
import warnings 
from .helpers import check_optype, check_trtype, breakeven
...
def single_plotter(op_type='c',spot=100, spot_range=10,strike=102,tr_type='b',op_pr=2, save=False, file='fig.png'):
    ...
    def plotter(x,y):
        ...
        plt.axvline(x=breakeven(x, y, op_type, strike, op_pr, multi=False), color='purple', linestyle='--', label=f'breakeven = {breakeven(x, y, op_type, strike, op_pr, multi=False):.2f}')
        plt.legend()
