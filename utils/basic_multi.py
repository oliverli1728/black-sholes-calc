...
from .helpers import payoff_calculator, check_optype, check_trtype, breakeven
...
def multi_plotter(spot_range=20, spot=100, notionals=[], fx=False,
    ...
    def plotter():
        ...
        for i in range (len(op_list)):
            ...
            if fx:
                sns.lineplot(x=x, y=y_list[i] * notionals[0], label=label, alpha=0.5)
            else:
                sns.lineplot(x=x, y=y_list[i], label=label, alpha=0.5)
            y+=np.array(y_list[i])
            
        plt.axhline(color='k', linestyle='--')
        plt.axvline(x=spot, color='r', linestyle='--', label=f'spot price = {spot}')
        plt.axvline(x=breakeven(x, y.tolist(), op_type, strike, op_pr, multi=True), color='purple', linestyle='--', label=f'breakeven = {breakeven(x, y.tolist(), op_type, strike, op_pr, multi=True):.2f}')
        ...
        if fx:
            plt.fill_between(x, y * notionals[0], 0, alpha=0.2, where=y>y0, facecolor='green', interpolate=True)
            plt.fill_between(x, y * notionals[0], 0, alpha=0.2, where=y<y0, facecolor='red', interpolate=True)
            sns.lineplot(x=x, y=y * notionals[0], label='combined', alpha=1, color='k')
        else:
