...
def steer_ols(swaps="", FX="", local_equity="", yc="", global_eq="", cb="", days=790, weights=[1], G10=False, US=False):
    ...
    zeros = pd.DataFrame(0, index=X.index, columns=["Signal"])
    df = zeros.join(fx3, how="left")
    df = df.join(fx4, how="left")
    df = df.join(fx, how="left")
    df = df.join(fx2, how="left")
...
df, z, y = steer_ols(swaps=["USSO2 Curncy", "TBSWH2 Curncy"], FX="USDTHB Curncy", yc="CTHAI1U5 Curncy", local_equity="SET Index", global_eq="MXWO Index", cb=["CL1 COMB Comdty"], weights=[1])
    ...
    def next(self):
        ...
        def parse_trades(trades):
            for x in trades:
                ...
                c = b[:10]
                d = datetime.strptime(c, date_format)
        ...
        if current_signal == 1 and abs(current_pct) > 0.01:
            ...
            self.sell(size=-1000, sl = current_price + current_price * (current_pct))
        
        elif current_signal == -1 and abs(current_pct) > 0.01:
            
            self.buy(size=1000, sl = current_price - current_price * (current_pct))

bt = Backtest(df, STEER, cash=100000)
stats = bt.run()
print(stats)
bt.plot()
