...
def steer_ols(swaps="", FX="", local_equity="", yc="", global_eq="", cb="", days=390, weights=[1], G10=False, US=False):
    ...
    fx.rename(columns={"E": "Close"}, inplace=True)
    fx2.rename(columns={"E": "Open"}, inplace=True)
    fx3.rename(columns={"E": "High"}, inplace=True)
    fx4.rename(columns={"E": "Low"}, inplace=True)
    if (G10):
...
df, z, y = steer_ols(swaps=["EUSA2 Curncy", "USSO2 Curncy"], FX="EURUSD Curncy", yc=["GTEUR10Y Govt", "EESWEC Curncy", "USGG10YR Index", "USSOC BGN Curncy"], local_equity="VTI US Equity", global_eq="MXWO Index", cb=["CL1 COMB Comdty"], weights=[1], G10=True)
...
bt = Backtest(df, STEER, cash=50000)
stats = bt.run()
print(stats)
bt.plot()
